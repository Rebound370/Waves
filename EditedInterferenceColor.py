#Colton Swartwoudt
#Interfering Waves with Color GIF Generator
...
def normalizeSingle(a):
    return int( 256 * ( a - -1.0 ) / (1 - -1.0) )
...
def main():
    ...
    while i < frameCount:
        ...
        valuesRGB = np.zeros( (WIDTH, HEIGHT, 3), dtype = np.uint8)
        for r in range(WIDTH):
            for c in range(HEIGHT):
                currentPos = Point(r, c)
                #currently only works with no more and no less then 3 waves, need to adjust
                #Takes each wave and applies it's height to either an R, G, or B value; and combines them.
                valuesRGB[r, c] = str( normalizeSingle( pattern.getWave(0).height(currentPos) ) ), str( normalizeSingle( pattern.getWave(1).height(currentPos) ) ), str( normalizeSingle( pattern.getWave(2).height(currentPos) ) )
        
        frameList.append( Image.fromarray(valuesRGB, "RGB") )
        i += 1
